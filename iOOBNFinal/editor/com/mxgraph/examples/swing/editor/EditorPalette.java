...
public class EditorPalette extends JPanel
{
			...
			String style, int width, int height, Object value)
	{
		...
		mxCell cell = new mxCell(value, geometry, style, null);// for edges, the BNNodeInfo is null
		cell.setEdge(true);
	...
	public void addTemplate(final String name, ImageIcon icon, String style,
			int width, int height, Object value, BasicNode bn)
	{
		mxCell cell = new mxCell(value, new mxGeometry(0, 0, width, height),
				style, bn);
		cell.setVertex(true);
		// this is not the place where nodes are added. I misunderstood
//		cell.setId(Integer.toString(mxGraphModel.nextId++));
//		System.out.println("In editor palette Cell ID " + cell.getId());
	...
	public void addTemplate(final String name, ImageIcon icon, mxCell cell)
	{
		...
		if (icon != null)
		{
			if (icon.getIconWidth() > 32 || icon.getIconHeight() > 30)
			{
//				icon = new ImageIcon(icon.getImage().getScaledInstance(32, 32, 0));
				icon = new ImageIcon(icon.getImage().getScaledInstance(32, 30, 0));
			}
		}

		final JLabel entry = new JLabel(icon);
		entry.setPreferredSize(new Dimension(30, 30));
