...
def setXaxisTitle(ax, xaxis, attrDict):
    ...
    if xaxis == 'NOC':
        ax.set_xlabel("Number of additional classes \n" + tailTitle)
        return "Number of additional classes " + tailTitle
    elif xaxis == 'NOO':
        ax.set_xlabel("Number of Object per additional class\n" + tailTitle)
        ...
        ax.set_xlabel("Number of States per node\n" + tailTitle)
        return "Number of States per node" + tailTitle
    elif xaxis == 'NOPar':
        ax.set_xlabel("Avg. Number of Parents\n" + tailTitle)
        return "Avg. Number of Parents" + tailTitle

ax = plt.gca()
def myPlot(df, xaxis, yaxis, yaxis2, attrDict, SET):
    # fig = plt.figure()
    # ax = fig.gca()
    ...
    ax = df[[xaxis, yaxis, yaxis2]].plot(x = xaxis, ax=SET)
    # ax = df[[xaxis, yaxis, yaxis2]].plot(row, col, num, x=xaxis)
...
X = [(2, 3, 1), (2, 3, 2), (2, 3, 3), (2, 3, 4), (2, 3, 5), (2, 3, 6)]
...
myPlot(df.loc[(df["NON"] == NONL) & (df["NOS"] == NOSL) & (df["NOPar"] == NOParL) & (df["NOO"] == NOOL)], 'NOC', 'SIIC', 'Hugin', attrDict, axes[0][0])
attrDict['NOC'] = NOCL

attrDict['NOO'] = None
myPlot(df.loc[(df["NON"] == NONL) & (df["NOS"] == NOSL) & (df["NOPar"] == NOParL) & (df["NOC"] == NOCL)], 'NOO', 'SIIC', 'Hugin', attrDict, axes[0][1])
attrDict['NOO'] = NOOL
...
attrDict['NOPar'] = NOParL
attrDict['NOS'] = NOSL
myPlot(df.loc[(df["NOS"] == NOSL) & (df["NOPar"] == NOParL) & (df["NOO"] == NOOL) & (df["NOC"] == NOCL)], 'NON', 'SIIC', 'Hugin', attrDict, axes[1][0])
attrDict['NON'] = NONL


attrDict['NOS'] = None
myPlot(df.loc[(df["NON"] == NONL) & (df["NOPar"] == NOParL) & (df["NOO"] == NOOL) & (df["NOC"] == NOCL)], 'NOS', 'SIIC', 'Hugin', attrDict, axes[1][1])
attrDict['NOS'] = NOSL

attrDict['NOPar'] = None
myPlot(df.loc[(df["NON"] == 5) & (df["NOS"] == NOSL) & (df["NOO"] == NOOL) & (df["NOC"] == NOCL)], 'NOPar', 'SIIC', 'Hugin', attrDict, axes[1][2])
attrDict['NOPar'] = NOParL
